...
def get_advantages(values, masks, rewards):
    ...
    return returns, (adv - np.mean(adv)) / (
                np.std(adv) + 1e-10)  # the 1e-10 residue is to make sure not to divide by zero
...
while not target_reached and iters < max_iters:
    ...
    for itr in range(ppo_steps):  # collect 128 interactions with the game
        state_input = K.expand_dims(state, 0)
        action_dist = model_actor.predict([state_input, dummy_n, dummy_1, dummy_1, dummy_1],
                                          steps=1)  # uses the actor model to predict the best actions
        q_value = model_critic.predict([state_input], steps=1)  # uses the critic model to predict the q value.
        action = np.random.choice(n_actions, p=action_dist[0,
                                               :])  # picks an action based on the action distribution from the actor model
    ...
    model_critic.fit([np.reshape(states, newshape=(128, 72, 96, 3))], [np.reshape(returns, newshape=(-1, 1))], shuffle=True, epochs=8, verbose=True)
    model_actor.fit(
        [np.reshape(states, newshape=(128, 72, 96, 3)), actions_probs, advantages, rewards, values[:-1]],
    ...
    # now we save the model checkpoints when we get a good model:
    print('total test reward=' + str(avg_reward))
    if avg_reward > best_reward:
        print("best reward=" + str(best_reward))
        model_actor.save('model_actor_{}_{}.hdf5'.format(iters, avg_reward))
        model_critic.save('model_actor_{}_{}.hdf5'.format(iters, avg_reward))
