...
while not target_reached and iters < max_iters:
    ...
    actor_loss = model_actor.fit(
            [np.reshape(states, newshape=(128, 72, 96, 3))], [np.reshape(actions_onehot, newshape=(-1, n_actions))], verbose=True, shuffle=True, epochs=8,
            callbacks=[tensor_board])
    critic_loss = model_critic.fit([np.reshape(states, newshape=(128, 72, 96, 3))], [np.reshape(returns, newshape=(-1, 1))], shuffle=True, epochs=8, verbose=True, callbacks=[tensor_board])
    ...
    #    model_actor.save("models/actor.h5")
     #   model_critic.save("models/critic.h5")
